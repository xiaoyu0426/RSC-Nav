...
### 2026-07-07: Demo-final backlog and real-world readiness gaps

当前 demo 的主目标是形成可展示的闭环：

```text
natural-language instruction
-> qwen3-max API planner
-> semantic waypoint / stopover sequence
-> Habitat navmesh execution
-> RGB grounding + depth + semantic evidence storyboard
```

但它还不是完整真实机器人闭环。后续优化事项固定为：

```text
1. Interest-driven exploration
   当前初入环境建图仍使用 scripted coverage loop。
   后续应替换为 exploration-interest / curiosity policy：
     unexplored frontier
     + semantic uncertainty
     + landmark novelty
     + task affordance prior
   输出应驱动 coverage / revisit / active observation，而不是固定路线。

2. Lingbo vision/depth model trial
   在现有 Habitat RGB-D / semantic oracle 之外，尝试灵波 vision 和 depth 模型。
   目标是评估它们能否替代或补强当前 grounding / depth / object evidence 前端。
   需要记录：
     RGB -> detection / grounding quality
     RGB -> depth scale stability
     object centroid projection error
     semantic evidence persistence in BEV

3. Real computed localization
   当前 Habitat live/demo 仍使用 simulator exact agent pose / sensor pose。
   真实世界版本必须替换为 SLAM / VIO / wheel odometry / relocalization 计算位姿。
   待解决问题：
     pose drift
     relocalization error
     camera-depth extrinsic calibration
     time synchronization
     map alignment confidence
   object memory confidence 应显式结合 semantic confidence 和 pose/map confidence。

4. API planner demo-level refinement
   当前 qwen3-max 已能基于 semantic map 选择 table / bed 等 waypoint。
   demo 级优化重点不是重写 planner，而是让自然语言任务链路更像真实助手：
     show current subgoal
     stop and look at selected waypoint
     verify target evidence
     explain uncertainty when cup/water labels are absent
     support negative evidence -> memory update -> replan
```
...
API key 不写入 `selected_phase5a_model.json` 或阶段文档；开发机运行时需要通过环境变量或本地未入库 secret 文件注入。
