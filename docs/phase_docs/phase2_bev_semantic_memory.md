# Phase 2: BEV and Semantic Memory
...
Phase 2 对应旧协议中的 BEV 记忆阶段，核心设计包括：

- RGB-D 到 BEV projection。
- occupancy map。
- explored map。
- confidence-weighted semantic map。
- semantic confidence decay。
- negative evidence。
- last_seen_time / freshness。
- stale flag / memory status。

当前实现已经把这些设计进一步扩展为 semantic BEV + object memory + recorder/report。
...
- 同一位置不同 pitch 的 object retention case。
- 快走 / 慢走 / 原地转向的 smooth weight 对照。
- same-context object removal / relocation 的最小扰动实验。
- semantic BEV 与 traditional BEV 的角色说明图。
...
- 在论文中作为 semantic-spatial memory substrate 和 update 机制可视化证据。
